...
4.  **Two-stage sampling** with PSU then dwelling
5.  **Two-stage sampling** with PSU then dwelling, with individual
    answers
...
- `pik_area`: probability of selection of the area (between
  ...
  $`r round(max(lfs_samp_area$pik_area), 4)`$ — nearly equal by design)
...
the total variance decomposes into two additive components:
...
When we use the wrapper, the data is automatically filtered with only
respondent units. A warning is shown to inform you.
...
E, whose difference yields the non-response variance (see Caron[^1], p.
...
[^1]: Caron N. (1998), "Le logiciel Poulpe : aspects méthodologiques",
    Actes des Journées de méthodologie statistique
    <http://jms-insee.fr/jms1998s03_1/> Deville, J.-C. (1993),
...
involves two stages. First, a stratified sample of municipalities is
drawn from the population. Then a random sample of dwellings is drawn.
Some of them do not respond, and answers are at dwelling level. At
last, weights are corrected through reweighting.
...
total number of PSUs in the population (or within a stratum, for the
stratified case), $n_P$ is the number of sampled PSUs, and
$\pi_{\text{psu}}$ is the first-order inclusion probability of a PSU. At
the dwelling level: $N_{h|j}$ is the number of dwellings in PSU $j$,
$n_{h|j}$ is the number of sampled dwellings in PSU $j$, and
$\pi_{h|j}$ is the conditional inclusion probability of dwelling $h$
given that PSU $j$ has been selected. The overall inclusion probability
of a dwelling is
...
$$V_{\text{NR}} = \sum_{h\in \text{resp}}{ \frac{1}{\pi_h} (1-p_h) \left(\frac{y_h}{p_h}\right)^2}$$
...
$$V_{PSU} = \sum_{strata} N_{P,s}^2 \left( 1 - \frac{n_{P,s}}{N_{P,s}} \right)\frac{s^2_{P,s}}{n_{P,s}}$$

Where $s^2_{P,s}$ is the sample variance of the $\hat Y_{j}$ values
within stratum $s$. In `gustave`, this is a single call to `var_srs()`
with the PSU-level strata.
...
introduce a custom experimental `qvar_ms()` function, to estimate
variance for multi-levels stratified simple random sampling.
...
var_sample_two <- function(y, lfs_samp_area, lfs_samp_dwel) {
  ...
  # Step 0: Prepare the calibration matrix (individual level)
  cal_form <- as.formula("~id_area")
  x_cal <- model.matrix(cal_form, data = resp_dwel)

  # Step 1: Replace y with calibration residuals
  ...
  # Step 2: Non-response variance (Poisson, individuals respondents only)
  variance[["nr"]] <- var_pois(
  ...
  # Step 3: Reintroduce dwellings non-respondents with zeros
  y <- add_zero(y, rownames = lfs_samp_dwel$id_dwel)
  
  # One variance per PSU, weighted by PSU size
  variance[["srs-dwel"]] <- unique(lfs_samp_dwel$id_area) %>% map(~{
    sub <- lfs_samp_dwel %>% filter(id_area == .x)
    pik_area <- unique(sub$pik_area)
    var_srs(
  ...
  y_area <- sum_by(y / lfs_samp_dwel$prep, by = lfs_samp_dwel$id_area, w = 1 / lfs_samp_dwel$pik_dwel)
...
The key addition is two aggregations, one at dwelling then at PSU
...
dwelling within each PSU, and then to divide by the selection
probability of each PSU. After that, we aggregate from dwelling to PSU
...
two-stage draw (stratified PSU sampling, then dwelling SRS within
PSUs), followed by an exhaustive collection of all individuals within
...
(by summing within dwellings) before computing the dwelling and PSU
...
lfs_samp_ind <- lfs_samp_ind %>% 
  select(-any_of(c("pik_area","w_area","w_dwel_nrc","w_dwel_cond","prep","rep","id_area"))) %>% 
  left_join(lfs_samp_dwel %>% 
              select(id_dwel,id_area,rep,prep,w_dwel_nrc,w_dwel_cond),by = "id_dwel") %>% 
...
var_sample_three_1 <- function(y, lfs_samp_area, lfs_samp_dwel, lfs_samp_ind) {
  ...
  # Step 3: Reintroduce indviduals non-respondents with zeros
  ...
  y_area <- sum_by(y_dwel, by = lfs_samp_dwel$id_area, w = 1 / lfs_samp_dwel$pik_dwel)
...
var_sample_three_2 <- function(y, lfs_samp_area, lfs_samp_dwel, lfs_samp_ind) {
  ...
  # Step 0: Prepare the calibration matrix (dwelling level)
  cal_form <- as.formula("~id_area")
  x_cal <- model.matrix(cal_form, data = resp_dwel)
  ...
  # Step 2: Non-response variance (Poisson, dwelling respondents)
  variance[["nr"]] <- var_pois(
  ...
  # One variance per PSU, weighted by PSU size
  variance[["srs-hh"]] <- unique(lfs_samp_dwel$id_area) %>% map(~{
    sub <- lfs_samp_dwel %>% filter(id_area == .x)
    pik_area <- unique(sub$pik_area)
    var_srs(
  ...
  # --- Transition: aggregate to PSU level ---
  y_area <- sum_by(y_dwel / lfs_samp_dwel$prep, by = lfs_samp_dwel$id_area, w = 1 / lfs_samp_dwel$pik_dwel)
  
  variance[["srs-psu"]] <- var_srs(
    y_area[lfs_samp_area$id_area, , drop = FALSE],
    pik    = lfs_samp_area$pik_area
  )
  ...
  diag_env_2$poulpe <- tibble(typeres="FP",value=Reduce(`+`, variance)) %>%
    ...
    add_row(typeres="F",value=poulpe_F) %>% 
    add_row(typeres="E",value=variance[["nr"]] - poulpe_F) %>% 
    add_row(typeres="P",value=variance[["srs-psu"]] + variance[["srs-hh"]]) %>% 
    add_row(typeres="S",value=NA) 
...
The point estimate is identical in both versions ($202\,636.6$), as it
...
practically equivalent** ($5.341 \times 10^8$ vs $5.343 \times 10^8$, a
$0.04\,\%$ difference). This is no accident. Since the response
probability is constant within each dwelling, expanding the individual
...
identical by construction ($5.217 \times 10^8$ and $1.133 \times 10^7$ in
both printouts). Only the non-response component itself differs:
$1.03 \times 10^6$ at the individual stage versus $1.32 \times 10^6$ at
the dwelling stage ($+28\,\%$). The dwelling-level Poisson term operates
...
$0.3\,\%$ of the uncalibrated total: for this design, *where the
...
Since stage 1 carried $97.7\,\%$ of the uncalibrated variance, the overall 
effect is dramatic — the variance is multiplied by $0.015$ (model 1) and 
$0.012$ (model 2). This is a deliberately extreme, didactic margin: 
...
($7.93 \times 10^6$ vs $6.41 \times 10^6$, CV $1.39\,\%$ vs $1.25\,\%$)
...
calibration then absorbs $82.3\,\%$ of the non-response component and
$45.4\,\%$ of the stage-2 sampling component. In model 1, the residuals
are taken at the individual stage and only reach stage 2 through
aggregation, absorbing less ($-73.2\,\%$ and $-32.4\,\%$). Residualizing
