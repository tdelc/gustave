...
knitr::opts_chunk$set(
  collapse = TRUE,
  comment = "#>",
  message = TRUE,  # qvar_ms messages and notes are part of the pedagogy
  warning = TRUE   # so are its warnings (single-unit strata, etc.)
)
```
...
dwellings). For each case, the `qvar_ms()` call is compared with a
hand-built variance wrapper (`define_variance_wrapper()`) that makes every
methodological step explicit: the hand-built version is the ground truth
showing exactly what `qvar_ms()` does under the hood.
...
* **Conditional weights.** `sampling_weight` and `nrc_weight` are
  **conditional** weights at the considered stage: the inverse of the
  inclusion (resp. response) probability *given the stages above* (Poulpe
  convention). This is the single most important convention of the
  interface.
...
| `nrc_weight`, `response_dummy`, `nrc_dummy` | yes (list) | conditional | last stage |
...
variance computed on the estimated area totals.
...
var_srs_two <- function(y, smp_area, smp_dwel) {
  ...
  variance[["srs-area"]] <- var_srs(y_area, pik = 1 / smp_area$w_area)
...
The `nr_stage*` and `sampling_stage*` components printed by the hand-built
version match the names used internally by `qvar_ms()`.
...
variance estimation and keeps them for point estimates.
...
lfs_samp_dwel$strata_dwel <- ifelse(lfs_samp_dwel$income < 10, "Q1", "Q2")
```
...
are excluded with a warning, just as at stage 1. This is expected and
frequent in household surveys.
...
* qualitative calibration variables (character or factor) are automatically
  ...
  `model.matrix()` (intercept + treatment contrasts) instead: the two
  parameterizations span the same column space, hence yield the same
  residuals and the same variance.
...
set.seed(123)
lfs_samp_ind$prep_ind <- runif(nrow(lfs_samp_ind))
lfs_samp_ind$rep_ind  <- lfs_samp_ind$prep_ind <= 0.8
...
a *different estimator* (with a different point estimate because without 
calibration step).
...
* *A non-zero calibration effect below the calibration stage.* Components
  ...
  Anything else indicates a bug or a misdeclared `calibration_level`.
...
* *A positive calibration effect on a variable supposedly related to the
  ...
  variables that the calibration variables explain (Särndal, Swensson and
  Wretman, 1992, ch. 6). A `+` effect on a target variable of the survey
  suggests reviewing the choice of margins.
...
`roh` estimates the intra-cluster correlation of the study variable and is
the key input for designing future waves of the survey. The model-based
justification of this decomposition is given by Gabler, Häder and Lahiri
(1999).
...
degrees of freedom (Korn and Graubard, 1999; Rust and Rao, 1996). The
normal-based confidence intervals produced by gustave implicitly assume
these degrees of freedom to be large. The `"dof"` diagnostic prints the
rule-of-thumb df and, when it is small, the width correction a Student-based
interval would apply:
...
versus, on a design with few primary units:

```
-- Degrees of freedom --
  df = 18 PSUs - 6 strata = 12
  Normal-based CIs are optimistic: a t-based CI would be 11.1 % wider.
```
...
* *df below ~30.* Normal intervals undercover; report t-based intervals or
  ...
  unstable and replication-based cross-checks are advisable (Wolter, 2007).
* *Domain estimation concentrated in few PSUs.* The nominal df is an upper
  ...
  is closer to the number of PSUs containing the domain (Korn and Graubard,
  1999, sec. 5.2). Cross-read with the `"domains"` diagnostic.
...
* *Very small minimum response probabilities.* A $\hat p_{\min}$ below
  ...
  single such unit can dominate a domain estimate. The usual remedies are
  collapsing reweighting cells or trimming (Little and Vartivarian, 2005,
  discuss when reweighting increases variance without reducing bias).
...
```{r diag-domains}
qvar6(lfs_samp_dwel,
  "Income, stratum Q1" = total(income, where = strata_dwel == "Q1"),
  "Income, stratum Q2" = total(income, where = strata_dwel == "Q2"),
  diagnostics = "domains"
)
```
...
* *Domains below a few dozen observations.* Both the estimate and its
  estimated variance are unstable; this is the natural boundary where
  design-based estimation gives way to small area methods (Rao and Molina,
  2015).
...
| calibration effect expected `+0.0 %` but non-zero below the calibration stage | misdeclared `calibration_level` or bug | check the wrapper with `inspect_wrapper_ml()` |
...
All the quantities above are by-products of the analytical (Taylor/Rao)
decomposition: they share its assumptions — invariance and independence
across stages, Poisson non-response within cells, first-order linearization
of calibration — and cannot validate those assumptions from within. The
standard external check is to re-estimate a few key variances with a
replication method (jackknife dropping one PSU at a time, or the Rao-Wu
rescaled bootstrap; Wolter, 2007; Rust and Rao, 1996): agreement between
the analytical and replicated variances is the strongest available evidence
that the design description encoded in the wrapper is faithful. This is a
natural extension for a future version.
...
definition log, and — most importantly — the exact variance estimation
...
## Quick inspection from the console

Two lightweight tools come first. Printing a `qvar_ms` wrapper displays
its definition call and the notes and warnings emitted when it was defined
(they are stored as attributes at definition time, see `qvar_notes()`):

```{r inspect-print, eval = TRUE}
qvar9               # print method: definition call + definition log
qvar_notes(qvar9)   # the same log, programmatically accessible
```

This is often enough to answer the most common question about a wrapper
someone else defined: *"which features does it take into account, and did
anything unusual happen at definition time?"* — the welcome message, the
automatic coercions, the strata crossing notes and the single-unit strata
warnings are all there.

## The full report

For anything deeper, call the inspector:

```{r inspect-call, eval = TRUE}
inspect_wrapper(qvar9)
```
...
The HTML report presents the same information in five cards, described
...
The technical parameters lists the switchable arguments of the
...
`inspect_wrapper()` invisibly returns everything it displays, as a
list with elements `reference`, `levels`, `calibration` and `pipeline`.
This makes the inspection scriptable — for example, a lightweight quality
gate in a production pipeline:

```{r inspect-programmatic, eval = FALSE}
info <- inspect_wrapper(qvar9, plot = FALSE)

# Fail early if any stage lost more than 5 % of its units to
# single-unit strata exclusion
excl_rate <- sapply(info$levels, function(l) l$n_excluded / l$n_units)
stopifnot(all(excl_rate < 0.05))

# Check that the response probabilities are within a plausible range
```

Combined with `qvar_notes()` for the definition log and the `diagnostics`
...
methodological one. The `diagnostics` technical parameter allows a quality control of the variance estimation procedure — variance components with 
...
`qvar_notes()` and `inspect_wrapper_ms()` make the wrapper itself readable: 
...
**The decomposition.** Writing $\hat{Y} = \sum_{i \in s_1} w_{1i}
\hat{Y}_i$ with $\hat{Y}_i$ the estimated totals of the primary units, the
implemented estimator is, for two stages,

$$\hat{V}(\hat{Y}) \;=\; \hat{V}_1\big(\{\hat{Y}_i\}\big) \;+\;
\sum_{i \in s_1} w_{1i}\, \hat{V}_{2i},$$

that is: the first-stage variance formula applied to the *estimated*
totals, plus the second-stage variance estimators weighted by the *first
power* of the first-stage weight (Rao, 1975; Särndal, Swensson and
Wretman, 1992, result 4.5.1). The generalization to more stages and to
non-response phases proceeds recursively, the Rao row weight at stage $k$
being the cumulated corrected weight of stages $1, \dots, k-1$. This is
the estimator implemented by the SAS Poulpe macros, and the reason why the
gustave variance functions expose a linear `w` argument.

**Assumptions.** The decomposition assumes the design is invariant and
independent across stages, simple random sampling (possibly stratified)
with equal probabilities within strata at each stage, and non-response
following a Poisson mechanism within the reweighting cells. `qvar_ms()`
enforces the equal-probability assumption stage by stage (mean weight per
stratum with a warning otherwise) and excludes single-unit strata, exactly
like `qvar()`.

**What to check first when results look off.** In order of likelihood:
(1) a conditional weight that is actually cumulated (`sampling_weight` or
`nrc_weight`) — watch the calibration g-ratio warning and the response
probability ranges; (2) a stage file restricted to respondents, whereas it
must contain all sampled units; (3) units attached to a non-responding
parent (blocked by an explicit check).

**Beyond qvar_ms.** More complex multistage cases — unequal probabilities
within strata, balanced sampling at the first stage, shared or integrated
...
* Gabler S., Häder S., Lahiri P. (1999), "A model based justification of
